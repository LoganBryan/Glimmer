...
namespace Utils
{
	...
	inline unsigned int GenerateCubemapCompressed(const std::vector<std::string>& faces)
	{
		unsigned int textureID;
		glGenTextures(1, &textureID);
		glBindTexture(GL_TEXTURE_CUBE_MAP, textureID);
		...
		for (unsigned int i = 0; i < faces.size(); i++)
		{
			...
			if (data)
			{
				glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_COMPRESSED_RGB_S3TC_DXT1_EXT, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
			}
			else
			{
				printf("Cubemap texture failed to load at %s", faces[i].c_str());
